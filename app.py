...
with st.sidebar:
    ...
    setting_no = st.text_input(
        "② 開始No",
        value="001",
        help="入力した桁数が維持されます（例: 001 → 001, 002...）"
    )
    
    # ③ 状態 (テキスト入力)
    setting_status = st.text_input(
        "③ 状態 (フリー入力)",
        value="",
        placeholder="例: 笑顔、会議中、食事"
    )
    
    st.info(f"作成イメージ:\n{setting_age}_{setting_no}_{setting_status if setting_status else '状態'}.jpg")
...
st.write("##### 画像アップロード")
st.caption("ルールに基づいて一括で名前を生成します。")
    ...
    if st.button(btn_label, type="primary"):
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            try:
                ...
                # --- ここを変更しました ---
                # ファイル名生成: 年代_No_状態
                # 状態が空欄の場合はアンダーバーが続かないように調整してもよいが、
                # 基本的には入力通り結合します
